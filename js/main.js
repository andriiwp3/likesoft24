$(document).ready(function () {
	...
	/**
	 * 
	 * Определяем поддержывает ли браузер формат изображений webp
	 *
 	 */
...
$('.categories__title').on('click', () => {
	if ($('.categories__title').hasClass('active')) {
		$('.categories__title').next().slideDown(300);
	} else {
		$('.categories__title').next().slideUp(300);
	}

	$('.categories__title').toggleClass('active');
	$('.categories__icon').toggleClass('active');
})
...
$('body').on('click', '.spoller', function (event) {
	...
	if (this.querySelector('.icon-menu')) {
		this.querySelector('.icon-menu').classList.toggle('active');
	}
...
$('body').on('click', '.tab__navitem', function (event) {
	...
	if (this.classList.contains('_product-type')) {
		let product = this.closest('._product');
		let productNumCounter = +product.querySelector('.number-product').textContent;

		changeSum(productNumCounter, product);
	}
});
...
/**
 * 
 * Логика для выбора количества товара
 * 
 */
let numberProductActions = document.querySelectorAll('.number-product__action');

numberProductActions.forEach(function (action) {
	action.addEventListener('click', () => changeProductNumber(action));
});

function changeProductNumber(action) {
	let productNumberCounter = action.closest('.number-product').querySelector('.number-product__num');
	let counterNum = +productNumberCounter.textContent;
	let product = action.closest('._product');

	if (action.classList.contains('number-product_plus')) {
		counterNum += 1;
		productNumberCounter.textContent = counterNum;
	} else {
		if (counterNum != 0) {
			counterNum -= 1;
			productNumberCounter.textContent = counterNum;
...
/**
 * 
 * Функция для смены суммы продукта
 *
 * @param number num кол-во товара
 * @param HTMLElement product  товар
 *
 */
function changeSum(num = 0, product = null) {
	if (product) {
		let blockWithPrice = product.querySelector('._prices .active') || product.querySelector('[data-price]');
		let price = blockWithPrice.dataset.price;
		let sum = price * num;

		blockWithPrice.textContent = sum + ' р';
	}

	if (document.querySelector('.sum')) {
		let totalSum = 0;
		let totalSumBlock = document.querySelector('.sum');
		let productsSumBlock = document.querySelectorAll('[data-price]');

		productsSumBlock.forEach(function (item) {
			let product = item.closest('._product');
			let productNumberCounter = +product.querySelector('.number-product__num').textContent;
			let productSum = productNumberCounter * item.dataset.price;


			totalSum += productSum;
		});

		totalSumBlock.innerHTML = 'Сумма заказа: <span>' + totalSum + ' р</span>';
	}
}
changeSum();
...
removeProduct.forEach(function (item) {
	item.addEventListener('click', function (e) {
		let parent = item.closest('._product');
		item.closest('._product').remove();
		changeSum();
	});
});
...
/**
 * 
 * Функция для скролла к блокам при нажатии на ссылку
 * 
 */
let gotoLinks = document.querySelectorAll('.goto');

gotoLinks.forEach(function (item) {
	item.addEventListener('click', function (e) {
		if (document.querySelector('.' + item.getAttribute('href').split('#')[1])) {
			e.preventDefault();
			window.scrollBy({
				top: document.querySelector('.' + item.getAttribute('href').split('#')[1]).getBoundingClientRect().top - document.querySelector('header').getBoundingClientRect().height,
				behavior: "smooth"
			});
...
/**
 * 
 * Логика для счетчика товаров в корзине
 * 
 */
let inCartBtns = document.querySelectorAll('.in-cart');
let cartCounter = document.querySelector('.cart-icon__counter');

inCartBtns.forEach(function (item) {
	item.addEventListener('click', function (e) {
		e.preventDefault();
		let counter = +cartCounter.textContent;

		counter += 1;
		cartCounter.textContent = counter;
		checkCounterValue(cartCounter);
	});
});
...
if ($('.testimonials__slider').length > 0) {
	$('.testimonials__slider').slick({
		...
		prevArrow: `<button type="button" class="testimonials__arrow arrow"><svg width="10" height="18" viewBox="0 0 10 18" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M9.35357 0.353516L0.868286 8.8388M9.64646 16.8389L1.16118 8.35359" stroke="#001A43"/></svg></button>`,
		nextArrow: `<button type="button" class="testimonials__arrow arrow"><svg width="10" height="18" viewBox="0 0 10 18" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M9.35357 8.35345L0.868286 16.8387M9.64646 8.8388L1.16118 0.353516" stroke="#001A43"/></svg></button>`,
		responsive: [
			{
				breakpoint: 991.98,
				settings: {
...
if ($('.similar__cards').length > 0) {
	$('.similar__cards').slick({
		autoplay: true,
		infinite: true,
		arrows: true,
		slidesToShow: 3,
		...
		//appendDots:
		//appendArrows:$('.mainslider-arrows .container'),
		prevArrow: `<button type="button" class="similar__arrow arrow"><svg width="10" height="18" viewBox="0 0 10 18" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M9.35357 0.353516L0.868286 8.8388M9.64646 16.8389L1.16118 8.35359" stroke="#001A43"/></svg></button>`,
		nextArrow: `<button type="button" class="similar__arrow arrow"><svg width="10" height="18" viewBox="0 0 10 18" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M9.35357 8.35345L0.868286 16.8387M9.64646 8.8388L1.16118 0.353516" stroke="#001A43"/></svg></button>`,
		responsive: [
			{
				breakpoint: 767.98,
				settings: {
 ...
 *
 */
$.each($('input._tel'), function (index, val) {
	$(this).attr('type', 'tel');
	$(this).focus(function () {
...
function formValidate(input) {
	...
	if (input.attr('type') == 'email' || input.hasClass('_email')) {
